...
def qafun_robotid_zero(knowledgeId, _question, robotId, status, top, com, robot_list, cos_list):
    return_list = []
    compare_dic = {}
    logger.info('qa _question :%s' % _question)
    ...
    try:
        ...
        if get_category['content'] != '':
            ...
            # if robotId == '0':
            #     for msg in _KVmsgs:
            #         if msg.knowledgeid == int(knowledgeId):
            #             compare_value = com.compare(statement=question, other_statement=msg.knowledge['cut_question'])
            #             com_dict[msg.knowledge['question_id']] = compare_value
            #             compare_dic[msg.knowledge['question_id']] = msg.knowledge['question']
            # else:
...
def qafun_robotid_zero_toponym(knowledgeId, _question, robotId, status, top, com, robot_list, cos_list, toponym):
    return_list = []
    compare_dic = {}
    logger.info('qa _question :%s' % _question)
    ...
    try:
        ...
        if get_category['content'] != '':
            ...
            get_addr_list = toponym.processSentenceRet(get_category['content'])
            logging.info("get_addr_list:%s" % str(get_addr_list))
            # 过滤词
            question = handlemsg.HandleMsg.cutMsg(get_category['content'])
            ...
            # 按照字典中的value大小排序，并且取出前top个字典，转成元组
            return_tuple = sorted(com_dict.items(), key=lambda e: e[1], reverse=True)[:top]
            logger.info("return_tuple:%s" % str(return_tuple))
            ...
            for _sotr in return_list_items:
                ...
                _questionId = _sotr[0]
                _sotr = _sotr[1]
                return_list.append({'questionType': '0', 'question': _question, 'questionId': _questionId, 'score': _sotr,'address':str(get_addr_list)})
...
#测例匹配方法
def ismatch_bak(testdetail_item, com, r_srinit, cos_wordvectors):
    get_data = qafun(_question=testdetail_item.example, robotId='0', status='1', top=1, com=com, robot_list=r_srinit, cos_list=cos_wordvectors)
    if len(get_data) == 0:
        return 0
    else:
...
def ismatch(testdetail_item, com, r_srinit, cos_wordvectors):
    get_data = qafun(_question=testdetail_item.example, robotId='0', status='1', top=1, com=com, robot_list=r_srinit, cos_list=cos_wordvectors)
    if len(get_data) == 0:
        return 2
    else:
...
# 测试用例执行方法
def configtestfun_bak(record_Id,com,r_srinit,cos_wordvectors):
    from sscp_py.models import AiTestDetail, AiTestRecord, AiTestResult
    import datetime
    # global testcase_runling_dict
    from sscp_py.views import testcase_runling_dict
    try:
        ...
        for testdetail_item in testdetail_list:
            ...
            if testdetail_item.question_id == current_question_id:
                current_question_id=testdetail_item.question_id
                match_result = ismatch(testdetail_item=testdetail_item, com=com, r_srinit=r_srinit, cos_wordvectors=cos_wordvectors)
...
def configtestfun(record_Id,com,r_srinit,cos_wordvectors):
    from sscp_py.models import AiTestDetail, AiTestRecord, AiTestResult
    import datetime
    # global testcase_runling_dict
    from sscp_py.views import testcase_runling_dict
    try:
        ...
        for testdetail_item in testdetail_list:
            ...
            if testdetail_item.question_id == current_question_id:
                ...
                if match_result == 0:
                    ...
                    return_dict["nomatchcount"] = return_dict["nomatchcount"] + 1
                    totla_nomathc_coiunt = totla_nomathc_coiunt + 1
                    testdetail_item.match_result = '无结果'
                    testdetail_item.save()
            else:
                logger.info("add AiTestResult msg:record_id=%d,question_id=%d,match_count=%d,nomatch_count=%d" % (
                    record_Id, return_dict["questionid"], return_dict["matchcount"], return_dict["nomatchcount"]))
        ...
        try:
            ...
            logger.error("AiTestRecord.objects.get(record_id=%s) ERROR:" % (str(record_Id), str(e)))
